...
```
git clone https://github.com/altixco/django-postgres-dokku parque_automotor
cd parque_automotor
```
...
```
make name=parque_automotor init
```
...
```
dokku apps:create parque_automotor
dokku postgres:create parque_automotor
dokku postgres:link parque_automotor parque_automotor
```
...
```
dokku config:set parque_automotor ENVIRONMENT=production DJANGO_SECRET_KEY=....
```
...
```
git remote add production dokku@<my-dokku-server.com>:parque_automotor
```
